...
pid_t		safePid; //already declared here!!
...

{
  ...
  isCoastClear	= 0;
  	//  DONE YOUR CODE HERE to send SIG_QUIT to both safePid and crackingThiefPid
  kill(safePid, SIG_QUIT);
  kill(crackingThiefPid, SIG_QUIT); //DELETED THIS ACCIDENTALLY FIXED!!!
}
				...
				)
{
  if  (isCoastClear)
  {
    ...
    haveTheGoods	= 1;
  	...
  	//  DONE YOUR CODE HERE to send SIG_QUIT to safePid
  kill(safePid, SIG_QUIT);
}
...
int		main		()
{
  ...
  if  (safePid == 0)//child
  {
  	//  DONE YOUR CODE HERE to start safe
  execl("./safe","safe",NULL);//execute safe
  fprintf(stderr,"Watching thief \"Where's the freaking safe?!\"\n");//if safe fails to run
    exit(EXIT_FAILURE);
  }
